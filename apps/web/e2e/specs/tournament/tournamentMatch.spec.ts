 ...
 *
 * 대결 조합은 서버 브래킷이 정하므로 목의 currentMatch / nextMatch 를 따른다.
 */
test('4강 두 매치를 진행하면 결승으로 전환되고, 결승 선택이 기록된다', async ({ page, api }) => {
  ...
  /** 4강 1번째 기록 — 서버가 같은 라운드의 다음 매치를 함께 내려준다 */
  api.post(ENDPOINTS.TOURNAMENT_MATCHES(2), {
    nextMatch: MOCK_MATCH_SEMI_SECOND,
    completed: null,
  });
  ...
  /** 결승 기록 — completed 에 1~4위 결과가 담겨 온다 */
  api.post(ENDPOINTS.TOURNAMENT_MATCHES(2), {
    nextMatch: null,
    completed: {
