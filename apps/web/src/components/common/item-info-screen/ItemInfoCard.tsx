...
function ItemInfoCard({ name, price, priceRefresh, onEdit }: ItemInfoCardProps) {
  const isRefreshing = priceRefresh?.isPending ?? false;

  return (
    <div className="relative rounded-xl bg-bg-layer-floating p-4">
      {/* 카드 높이가 흔들리지 않도록 갱신 중에도 기존 내용이 자리를 지킨다 */}
      <div className={cn(isRefreshing && 'invisible')}>
